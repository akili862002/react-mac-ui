...
export interface ListActions<T> {
  ...
  removeItem: (item: T, isEqual: (a: T, b: T) => boolean) => void;
  /**
...
export function useList<T>(
  ...
  const actions = useMemo<ListActions<T>>(() => {
    const a = {
      ...
      removeItem: (item: T, isEqual: (a: T, b: T) => boolean) => {
        actions.set((curr) => {
          const arr = curr.filter((_item) => isEqual(item, _item));
          return arr;
        });
      },
